...
void mainUpdate()
{
    ...
    g_CounterRot += 0.03f;
    //pg::Scene::GetInstance()->GetMainDirectionalLight().SetDirection({ 0.0f, glm::cos(g_CounterRot) , glm::sin(g_CounterRot) });
    pg::Scene::GetInstance()->GetMainDirectionalLight().SetDirection({ 0.0f, 0.0f , 1.0f });
